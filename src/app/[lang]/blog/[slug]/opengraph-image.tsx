...
export const alt = "TRIA Lab blog post"
export const size = openGraphImageSize
export const contentType = "image/png"

export function generateStaticParams() {
  return locales.flatMap((lang) => getDirMetadata(lang, "blog").map(({ slug }) => ({ lang, slug })))
}
...
export default async function Image({
  ...
  const post = getFileMetadata(lang, `blog/${slug}`)
  const { t } = await getTranslation(lang)

  return new ImageResponse(
    <OpenGraphImage label={t("page.blog")} title={post.title} detail={post.date} />,
    size,
  )
}
